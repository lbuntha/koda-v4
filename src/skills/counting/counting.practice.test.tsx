...
describe("practice lessons", () => {
  it("there is one per engine", () => {
    ...
    expect(questions).toBeGreaterThanOrEqual(40);
  });
  ...
  it.each(practiceLessons)("$id is open from the start", (lesson) => {
    // A child who already knows the technique should not have to sit through
    // the lesson to reach the questions.
  ...
  it.each(practiceLessons)("$id names more questions than it has modes", (lesson) => {
    ...
    // Base-10 has no modes: its lessons differ by how big the number is, so
    // practice there is the whole range rather than a cycle.
    expect(questionsPerRound).toBeGreaterThanOrEqual(modes?.length ?? 1);
  });
