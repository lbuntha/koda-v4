  ...
  return (
    <Card className="min-h-56 overflow-hidden rounded-3xl border-0 bg-white shadow-[0_12px_34px_-24px_rgba(39,51,74,0.5)] dark:bg-white/[0.045] dark:shadow-none">
      <CardContent className="flex min-h-56 flex-col p-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-xs font-bold text-[#8792A5] dark:text-[#9AA3B5]">
                  {label(child.grade_level)} · {label(child.primary_subject)}
                </p>
              </div>
              <div className="flex shrink-0 gap-0.5">
                <Button type="button" variant="ghost" size="icon" onClick={onProgress} aria-label={`View ${child.name}'s progress`} className="h-9 w-9 rounded-xl text-[#7C72A0] hover:text-[#6844EA] dark:text-[#AFA6C8]">
                  <BarChart3 size={16} />
                </Button>
                <Button type="button" variant="ghost" size="icon" onClick={onEdit} aria-label={`Edit ${child.name}`} className="h-9 w-9 rounded-xl text-[#7C72A0] hover:text-[#6844EA] dark:text-[#AFA6C8]">
                  <Pencil size={15} />
                </Button>
                {showRemove && (
                  <Button type="button" variant="ghost" size="icon" onClick={onRemove} aria-label={`Remove ${child.name}`} className="h-9 w-9 rounded-xl text-[#9A91AD] hover:text-rose-600 dark:text-[#AFA6C8] dark:hover:text-rose-300">
                    <Trash2 size={15} />
                  </Button>
                )}
              </div>
        ...
        <Button type="button" onClick={onPlay} className="mt-auto w-full rounded-xl bg-[#7252D8] text-xs font-extrabold hover:bg-[#6546CC]">
