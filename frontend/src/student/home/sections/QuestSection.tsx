    ...
    <section id="kid-quests" className="mt-6 scroll-mt-24">
      ...
      <Tabs value={tab} onValueChange={setTab} className="mt-4">
        ...
        <TabsContent value="daily" className="mt-4">
          <div className="grid gap-3 rounded-2xl bg-[linear-gradient(135deg,#EEE9FF_0%,#EFF7FF_100%)] p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-5 dark:bg-[linear-gradient(135deg,rgba(112,80,216,0.2),rgba(39,119,184,0.12))]">
            <div className="flex min-w-0 items-center gap-3.5">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-[#6844EA] shadow-sm dark:bg-white/10 dark:text-[#CDBEFF]">
          ...
          <div className="mt-3 space-y-2.5">
            {activities.length > 0 ? activities.map((item, index) => {
              ...
              return (
                <Card key={`${item.assignmentId}:${item.skillId}`} className="overflow-hidden rounded-2xl border-0 bg-white shadow-none dark:bg-white/[0.035]">
                  <CardContent className="flex items-center gap-3 p-3 sm:px-4 sm:py-3.5">
                    ...
                    <Button type="button" size="sm" variant={done ? "ghost" : "default"} onClick={() => onStart(item)} className={`min-w-20 rounded-full text-[11px] font-extrabold sm:min-w-24 ${done ? "text-emerald-600 dark:text-emerald-300" : "bg-[#7252D8] hover:bg-[#6546CC]"}`}>
              ...
              <div className="rounded-2xl bg-white px-4 py-10 text-center dark:bg-white/[0.035]">
        ...
        {(["weekly", "challenges"] as const).map(value => {
          ...
          return (
            <TabsContent key={value} value={value} className="mt-4">
              <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl bg-white px-5 text-center dark:bg-white/[0.035]">
