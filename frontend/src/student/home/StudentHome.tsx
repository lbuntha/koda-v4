...
export const StudentHome: React.FC<StudentHomeProps> = ({
  ...
  const goal =
    course.mode === "scheduled" && course.queue.length > 0 ? (
      <span className="hidden items-center gap-1.5 rounded-full border border-[#E2DEEF] bg-white px-3 py-1.5 text-xs font-semibold text-[#6B57D8] sm:inline-flex">
        <Target size={14} /> {course.queue.length} to practise today
      </span>
    ) : null;

  return (
    <div className="min-h-screen bg-[#F8F7FC] text-[#17152F]" data-band="student">
      <HomeHeader studentName={studentName} studentAvatar={studentAvatar} onExit={onExit} right={goal} />
