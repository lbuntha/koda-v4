...
export const HeroActivity: React.FC<Props> = ({ item, canSkip, skipping, onStart, onSkip }) => {
  ...
  return (
    <Card className="border-[#E2DEEF] shadow-[0_12px_36px_rgba(45,35,100,0.08)]">
      <CardContent className="flex flex-col items-center p-7 text-center md:p-10">
        ...
        <h2 className="mt-5 text-2xl font-extrabold text-[#22203A] md:text-3xl">{item.skillLabel}</h2>
        <p className="mt-2 max-w-md text-sm leading-relaxed text-[#746E8D]">{item.reason || "Let’s practise!"}</p>
        ...
        {canSkip && (
          <button
            ...
            disabled={skipping}
            onClick={() => onSkip(item)}
            className="mt-3 text-xs font-semibold text-[#9B95B2] hover:text-[#6B57D8] disabled:opacity-60"
          >
