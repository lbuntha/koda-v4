...
## 📲 Installable App / Offline (`/src/pwa/`)

Koda ships as a PWA: installable to a home screen, and playable through a dropped connection.

*   **Manifest + service worker** are generated by `vite-plugin-pwa` — configured entirely in `vite.config.ts`, nothing is hand-written into `dist/`. Icons live in `/public/icons/` (`icon-*` for `any`, `icon-maskable-*` padded for platform masks, `apple-touch-icon.png` flattened for iOS).
*   **Precache = shell only** (`js/css/html/woff2` + icons, ~2.7 MB). The ~12 MB of artwork in `/public/assets/` is deliberately *excluded* and cached at runtime (`CacheFirst`, 30 days) as a child meets it, so a first install stays small.
*   **API *data* is never cached** — only files. `/api/**` is on the navigation-fallback denylist, and the one API route with a runtime cache is `/learning/assets/**`, which serves published artwork rather than JSON.
*   **A signed-in learner can finish their session offline.** Three pieces make that work, and all three are load-bearing:
    1.  `OfflineError` in `src/api/client.ts` separates "the server never answered" from "the server said no". **Never collapse them back together** — `AuthContext` clears the session on a 401 and keeps it on an `OfflineError`, and conflating the two signs a child out of an installed app the moment they open it without a network.
    2.  `src/api/offlineCache.ts` keeps the last successfully-read account, course and progress (versioned, 7-day expiry, cleared on logout). It is only ever *read* when the server is unreachable — a cached plan must never shadow a live one. A `/learning/today` payload carries its questions inline, so one cached course plus the precached canvases is a complete session.
    3.  `src/pwa/warmAssetCache.ts` pulls the artwork for today's queue into the runtime cache while the network is still up, because `CacheFirst` otherwise only holds pictures the child has already been shown.
*   **Storage is requested as persistent** (`src/pwa/persistentStorage.ts`) once a learner has a saved plan. Cache Storage and localStorage are evictable by default, and an eviction would take the precached shell, the cached plan *and* the unsynced attempt outbox at once. Asked after the first cached plan rather than at app start, because Firefox shows a permission prompt.
*   **iOS status bar is `default`, not `black-translucent`.** Translucent starts the web view behind the status bar, and every top header here is `sticky top-0` with no top inset — the clock and the header collided on notched iPhones. Landscape cutouts are handled by side insets on `body` under `@media (display-mode: standalone)` in `index.css`.
*   Attempts made offline are already handled by the existing `analyticsLogger` outbox (localStorage + an `online` listener). The player re-reads the plan from the server on reconnect. **Known gap:** an activity completed offline still shows as pending in the cached queue until that resync.
*   **Updates are offered, never forced** (`registerType: 'prompt'`). A waiting worker surfaces a "Reload" toast from `PwaPrompts.tsx`; the page is never swapped under a child mid-activity. Long-lived tabs re-check hourly.
*   **`PwaPrompts.tsx`** is mounted once in `main.tsx` and owns all three notices — update ready, offline, and the install suggestion (`useInstallPrompt.ts`: second visit onwards, dismissal remembered 30 days, iOS gets Share-sheet instructions since it has no `beforeinstallprompt`).
*   **Development**: the worker is off by default so edits are never hidden behind a stale shell. `VITE_PWA_DEV=true npm run dev` to exercise it; otherwise test with `npm run build && npm run preview`.
*   **`build.emptyOutDir` must stay `true`.** A service worker reads `dist/`, so leftover chunks from a previous build end up in the precache manifest and the worker downloads *two* generations of the app (measured: 154 entries / 2.7 MB → 254 / 4.5 MB). If a build fails with `EACCES` on `dist/sw.js`, the cause is a root-owned `dist` from a `sudo` build — clear it with `sudo rm -rf frontend/dist`, don't disable `emptyOutDir`.
*   **Zoom is locked to the activity surface only** (`src/hooks/useZoomLock`, applied to the `GameLauncher` root). Registering those handlers on `window` also removes ⌘/Ctrl-scroll zoom from the parent, admin and studio screens — a WCAG 1.4.4 failure for adults. `useZoomLock.test.tsx` pins both halves.
*   **Hosting** must serve `sw.js`, `index.html` and `manifest.webmanifest` uncached — already set in both `firebase.json` and `nginx.conf`. Breaking that pins users to an old deploy.
